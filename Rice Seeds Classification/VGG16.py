...
def main():
    ...
    base_model.classifier[6] = nn.Sequential(
        nn.Linear(base_model.classifier[6].in_features, 384),
        nn.ReLU(),
        nn.Dropout(0.5),
        nn.Linear(384, 1),  # Single output for binary classification
    )
    ...
    # optimizer = optim.Adam(base_model.parameters(), lr=1e-5, weight_decay=1e-3)  # Added weight decay for regularization
    ...
    # optimizer = optim.AdamW(base_model.parameters(), lr=1e-5, weight_decay=1e-2) #AdamW optimizer
    ...
    def plot_confusion_matrix(model, dataloader):
        ...
        plt.tight_layout()
        plt.savefig("confusion_matrix le-5 VGG RMSprop2.png")
        plt.show()
    ...
    # Save the model
    print("Confusion matrix saved to 'confusion_matrixVGG RMSprop3.png'")
    model_save_path = './Fyp_VggRMSprop3.pth'  # Update this path if necessary
